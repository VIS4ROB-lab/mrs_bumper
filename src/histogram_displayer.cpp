...
class HistogramDisplayer : public mrs_lib::Node {
 public:
  HistogramDisplayer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
      : mrs_lib::Node("histogram_displayer", options) {
    ...
    sh_hist = mrs_lib::SubscriberHandler<mrs_msgs::msg::Histogram>(
        shopts, "~/histogram", rclcpp::Duration::from_seconds(5.0));

    timer_ = this_node_ptr()->create_wall_timer(
        std::chrono::milliseconds(33), [this]() {
          if (sh_hist.newMsg()) {
            ...
            cv::Mat disp_im =
                cv::Mat::zeros(hr + bot_rows, hc + right_cols, CV_8UC3);
            cv::Mat hist_roi =
                disp_im(cv::Rect(cv::Point(0, 0), cv::Size(hr, hc)));
 ...
 private:
  ...
  static void mouse_callback([[maybe_unused]] int event, int x, int y,
